...
import (
	...
	"github.com/fromil88/gomobile/bind"
	"github.com/fromil88/gomobile/internal/importers"
	"github.com/fromil88/gomobile/internal/importers/java"
	"github.com/fromil88/gomobile/internal/importers/objc"
...
func genPkg(lang string, p *types.Package, astFiles []*ast.File, allPkg []*types.Package, classes []*java.Class, otypes []*objc.Named, libName string) {
	...
	case "java":
		...
		if p == nil {
			dir, err := packageDir("github.com/fromil88/gomobile/bind")
			if err != nil {
				errorf(`"github.com/fromil88/gomobile/bind" is not found; run go get github.com/fromil88/gomobile/bind: %v`, err)
				return
			}
			repo := filepath.Clean(filepath.Join(dir, "..")) // github.com/fromil88/gomobile directory.
			...
			javaDir, err := packageDir("github.com/fromil88/gomobile/bind/java")
			if err != nil {
				errorf("unable to import bind/java: %v", err)
				return
			}
	...
	case "objc":
		...
		if p == nil {
			// Copy support files
			dir, err := packageDir("github.com/fromil88/gomobile/bind/objc")
			if err != nil {
