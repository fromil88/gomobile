...
func runVersion(cmd *command) (err error) {
	// Check this binary matches the version in github.com/fromil88/gomobile/cmd/gomobile
	...
	version, err := func() (string, error) {
		...
		cmd := exec.Command("go", "list", "-f", "{{.Stale}}", "github.com/fromil88/gomobile/cmd/gomobile")
...
func mobileRepoRevision() (rev string, err error) {
	b, err := exec.Command("go", "list", "-f", "{{.Dir}}", "github.com/fromil88/gomobile/app").CombinedOutput()
	if err != nil {
