...
func runBuildImpl(cmd *command) (*packages.Package, error) {
	...
	if !nmpkgs["github.com/fromil88/gomobile/app"] {
		return nil, fmt.Errorf(`%s does not import "github.com/fromil88/gomobile/app"`, pkg.PkgPath)
	}
...
func extractPkgs(nm string, path string) (map[string]bool, error) {
	if buildN {
		return map[string]bool{"github.com/fromil88/gomobile/app": true}, nil
	}
	r, w := io.Pipe()
