...
func TestBindAndroid(t *testing.T) {
	...
	for _, tc := range tests {
		...
		cmdBind.flag.Parse([]string{"github.com/sagernet/gomobile/asset"})
		err := runBind(cmdBind)
		if err != nil {
			t.Log(buf.String())
			t.Fatal(err)
...
WORK=$WORK
GOOS=android CGO_ENABLED=1 gobind -lang=go,java -outdir=$WORK{{if .JavaPkg}} -javapkg={{.JavaPkg}}{{end}} github.com/sagernet/gomobile/asset
...
GOOS=ios CGO_ENABLED=1 gobind -lang=go,objc -outdir=$WORK/ios -tags=ios{{if .Prefix}} -prefix={{.Prefix}}{{end}} github.com/sagernet/gomobile/asset
...
func TestBindWithGoModules(t *testing.T) {
	...
	if out, err := exec.Command("go", "build", "-o="+dir, "github.com/sagernet/gomobile/cmd/gobind").CombinedOutput(); err != nil {
		t.Fatalf("%v: %s", err, string(out))
	}
	if out, err := exec.Command("go", "build", "-o="+dir, "github.com/sagernet/gomobile/cmd/gomobile").CombinedOutput(); err != nil {
		t.Fatalf("%v: %s", err, string(out))
	}
	path := dir
	...
	for _, target := range []string{"android", "ios"} {
		t.Run(target, func(t *testing.T) {
			...
			tests := []struct {
				...
				{
					Name: "Absolute Path",
					Path: "github.com/sagernet/gomobile/bind/testdata/cgopkg",
				},
				{
					Name: "Relative Path",
